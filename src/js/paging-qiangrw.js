	...
	function do_show_page_idx() {
		...
		if(do_cur_page <= 5){
			for(var i = 1; i < do_cur_page; i ++)
				str_content += ('<a name="do_page_idx">'+i+'</a></li>&nbsp;');
			str_content += ('<li class="active"><a name="do_page_idx">'+do_cur_page+'</a></li>&nbsp;');
		}
		else{
			if(do_cur_page >= do_total_page - 4){
				for(var i = ((do_total_page - 6) < 1?1:(do_total_page - 6)); i <=do_cur_page - 3; i ++)
					str_content += ('<a name="do_page_idx">'+i+'</a></li>&nbsp;');
				if(do_total_page > 9){
					str_content = '<a name="do_page_idx">...</a></li>&nbsp;' + str_content;
					str_content = '<a name="do_page_idx">1</a></li>&nbsp;' + str_content;
				}else{
					for(var i = do_total_page - 7; i >= 1; i --)
						str_content = ('<a name="do_page_idx">'+i+'</a></li>&nbsp;') + str_content;
				}
					
			}else{
				str_content += '<a name="do_page_idx">1</a></li>&nbsp;';
				str_content += '<a name="do_page_idx">...</a></li>&nbsp;';
			}
		...
		if(do_cur_page >= do_total_page - 4){
			...
			if(do_cur_page <= 5){
				...
				if(do_total_page>9){
					str_content += ('<a name="do_page_idx">...</a></li>&nbsp;');
					str_content += ('<a name="do_page_idx">'+do_total_page+'</a></li>&nbsp;');
				}else{
					for(var i = 8; i <=do_total_page; i ++)
						str_content += ('<a clanamess="do_page_idx">'+i+'</a></li>&nbsp;');
				}
			}else{
				str_content += ('<a name="do_page_idx">...</a></li>&nbsp;');
				str_content += ('<a name="do_page_idx">'+do_total_page+'</a></li>&nbsp;');
		...
		if(do_cur_page!=1)
			str_content=('<a name="do_page_idx">上一页</a></li>&nbsp;')+str_content;
		if(do_cur_page!=do_total_page)
			str_content+=('<a name="do_page_idx">下一页</a></li>&nbsp;');
