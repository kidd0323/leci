...
1.支付宝接口中需要修改参数的文件有 alipaydefault.aspx文件;买家付款成功后反馈的页面时 return_url(页面跳转),notify_url(后台通知);AliPay.cs不用修改.  alipaydefault.aspx文件中 seller_email,key,partner 对应的分别是支付宝帐号,安全校验码和合作id 
(注：如何获取安全校验码和合作ID
a.访问 www.alipay.com，然后登陆您的帐户.
b.点击右上角的“商家服务”.

 alipaydefault.aspx页面,需要传递参数到这个页面,需要传递的参数是 subject(商品名称),body(商品说明),price,out_trade_no(外部商家订单号.为了方便测试,我在alipaydefault.aspx这个页面采用的是以日期的形式的数字字符串,这个订单号必须保证提交给支付宝的时候是唯一的,否则支付不了
